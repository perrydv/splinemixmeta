...
test_that("splinemixmeta perfectly passes through to mixmeta if no smooth is specified", {
  ...
  expect_equal(res_splinemix$coefficients, res_mixmeta$coefficients, tolerance = 1e-6)
  expect_equal(res_splinemix$Psi, res_mixmeta$Psi, tolerance = 1e-6)
...
test_that("splinemixmeta works with a simple smooth in one x", {
  ...
  expect_equal(AIC(res_splinemixA), AIC(res_splinemixB), tolerance = 1e-6)
  expect_equal(AIC(res_splinemixA), AIC(res_splinemixC), tolerance = 1e-6)
  expect_equal(AIC(res_splinemixA), AIC(res_splinemixD), tolerance = 1e-6)
  ...
  expect_equal(predA[,'blup'], predB[,'blup'], tolerance = 1e-6)
  expect_equal(predA[,'blup'], predC[,'blup'], tolerance = 1e-6)
  expect_equal(predA[,'blup'], predD[,'blup'], tolerance = 1e-6)
  expect_equal(predA[,'se'], predB[,'se'], tolerance = 1e-6)
  expect_equal(predA[,'se'], predC[,'se'], tolerance = 1e-6)
  expect_equal(predA[,'se'], predD[,'se'], tolerance = 1e-6)
...
test_that("splinemixmeta works with two smooths", {
  ...
  expect_equal(res, res2, tolerance = 1e-6)
...
test_that("splinemixmeta gives correct answer for SFB30AO data", {
  ...
  package_resB <- splinemixmeta(smooth = mgcv::s(year, k = 10, bs = "cr"),
                               ...
                               data = SFB30A0,
                               se = SFB30A0$se)
  expect_equal(AIC(orig_res$fit), AIC(package_resA), tolerance = 1e-6)
  expect_equal(AIC(orig_res$fit), AIC(package_resB), tolerance = 1e-6)
  expect_equal(as.numeric(orig_res$fit$coefficients),
               as.numeric(package_resB$coefficients), tolerance = 1e-6)
